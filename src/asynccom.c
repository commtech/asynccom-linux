...
int  asynccom_write(struct tty_struct *tty, struct usb_serial_port *port, const unsigned char *user_buffer, int count)
{
    ...
    usb_bulk_msg(serial->dev, usb_sndbulkpipe(serial->dev, 6), buf, (count * 2), sent, 0);
    kfree(buf);
    kfree(sent);
    return count;
}
...
static void asynccom_set_termios(struct tty_struct *tty, struct usb_serial_port *port, struct ktermios *old_termios)
{
    ...
    /*stop bits*/
    if(cflag & CSTOPB) urb_value |= 0x04;
    /*Parity bits*/
    if(cflag & PARENB) urb_value |= 0x08;
    if(cflag & PARODD) urb_value |= 0x10;
...
void set_register(struct usb_serial_port *port, int addr, int val)
{
    ...
    msg = kmalloc(sizeof(char) * 7, GFP_KERNEL);
    count = kmalloc(sizeof(int), GFP_KERNEL);

    msg[0] = (char)(0x6A); //write command
    msg[1] = (char)(addr >> 8);
    msg[2] = (char)(addr);
    msg[3] = (char)(val >> 24);
    msg[4] = (char)(val >> 16);
...
__u32 get_register(struct usb_serial_port *port, int addr)
{
    ...
    msg = kmalloc(sizeof(char) * 3, GFP_KERNEL);
    count = kmalloc(sizeof(int), GFP_KERNEL);
    val = kmalloc(sizeof(__u32), GFP_KERNEL);
    ...
    usb_bulk_msg(serial->dev, usb_sndbulkpipe(serial->dev, 1), msg, sizeof(msg), count, HZ*10);
    usb_bulk_msg(serial->dev, usb_rcvbulkpipe(serial->dev, 81), val,  sizeof(val), count, HZ*10);

    out_val = ((*val>>24)&0xff) | ((*val<<8)&0xff0000) | ((*val>>8)&0xff00) | ((*val<<24)&0xff000000);
    kfree(val);
    kfree(msg);
    kfree(count);
    return out_val;
...
void asynccom_port_set_clock_bits(struct usb_serial_port *port, unsigned char *clock_data)
{
    ...
    for (i = 19; i >= 0; i--) {
        for (j = 7; j >= 0; j--) {
            ...
            if (bit) new_fcr_value |= dta_value; /* Set data bit */
            else new_fcr_value &= ~dta_value; /* Clear data bit */
    ...
    {
        buf_data = data[i];
        set_register(port, 0x0040, buf_data);
    }
    kfree(data);
}
							...
							size_t count)
{
    ...
    if(value == 1) asynccom_enable_echo_cancel(port);
    if(value == 0) asynccom_disable_echo_cancel(port);
